 ...
 */
enum class SerializationFormat : uint8_t { JSON = 0, AVRO, PROTOBUF, BINARY, CSV, CUSTOM };
 ...
 */
enum class CompressionType : uint8_t { NONE = 0, GZIP, SNAPPY, LZ4, ZSTD };
...
public:
    ...
    ipb::common::Result<std::unordered_map<std::string, std::string>> get_topic_metadata(
    ...
    using CustomPartitioner =
        std::function<int32_t(const ipb::common::DataPoint&, int32_t num_partitions)>;
...
private:
    ...
    std::string generate_message_key(const ipb::common::DataPoint& data_point,
                                     const TopicConfig& topic_config);
    int32_t determine_partition(const ipb::common::DataPoint& data_point,
                                const TopicConfig& topic_config);
...
public:
    ...
    static std::unique_ptr<KafkaSink> create_simple(
        const std::vector<std::string>& bootstrap_servers, const std::string& topic_name);
    static std::unique_ptr<KafkaSink> create_secure(
        const std::vector<std::string>& bootstrap_servers, const std::string& topic_name,
        const std::string& username, const std::string& password);

    // Preset factories
    static std::unique_ptr<KafkaSink> create_high_throughput(
        const std::vector<std::string>& bootstrap_servers, const std::string& topic_name);
    static std::unique_ptr<KafkaSink> create_low_latency(
        const std::vector<std::string>& bootstrap_servers, const std::string& topic_name);
    static std::unique_ptr<KafkaSink> create_exactly_once(
        const std::vector<std::string>& bootstrap_servers, const std::string& topic_name);
};
