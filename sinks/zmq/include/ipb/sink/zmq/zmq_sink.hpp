...
public:
    ...
    ipb::common::Result<> set_curve_keys(const std::string& public_key, const std::string& secret_key);
...
public:
    ...
    // Preset factories
    static std::unique_ptr<ZMQSink> create_high_throughput(const std::string& address, uint16_t port);
