...
SyslogSink::SyslogSink(const SyslogSinkConfig& config) : config_(config) {}
    ...
    try {
        ...
        if (config_.fallback_config.enable_file_fallback) {
            fallback_file_stream_ = std::make_unique<std::ofstream>(
                config_.fallback_config.fallback_file_path, std::ios::out | std::ios::app);
        }
...
common::Result<void> SyslogSink::start() {
    if (running_.load()) {
        return common::Result<void>(common::ErrorCode::ALREADY_EXISTS,
                                    "Syslog sink is already running");
    }
...
std::string SyslogSink::format_message(const common::DataPoint& data_point,
                                       SyslogPriority priority) const {
...
std::string SyslogSink::format_rfc5424(const common::DataPoint& data_point,
                                       SyslogPriority priority) const {
    ...
    // Message
    oss << "Protocol=" << data_point.get_protocol_id() << " Address=" << data_point.get_address()
...
std::string SyslogSink::format_rfc3164(const common::DataPoint& data_point,
                                       SyslogPriority priority) const {
    std::ostringstream oss;
...
std::string SyslogSink::format_cef(const common::DataPoint& data_point,
                                   SyslogPriority priority) const {
    std::ostringstream oss;

    // CEF header
    oss << "CEF:0|IPB|Gateway|1.0|" << data_point.get_protocol_id() << "|DataPoint|"
        << (10 - static_cast<int>(priority)) << "|";  // CEF severity (0-10, inverted from syslog)

    // Extension fields
    oss << "src=" << get_hostname() << " dst=" << data_point.get_address()
...
std::string SyslogSink::format_leef(const common::DataPoint& data_point,
                                    SyslogPriority priority) const {
    std::ostringstream oss;
    ...
    oss << "src=" << get_hostname() << "\tdst=" << data_point.get_address()
        << "\tproto=" << data_point.get_protocol_id() << "\tsev=" << static_cast<int>(priority)
        << "\tquality=" << static_cast<int>(data_point.get_quality());
...
std::string SyslogSink::format_plain(const common::DataPoint& data_point,
                                     SyslogPriority priority) const {
    std::ostringstream oss;

    oss << "[" << static_cast<int>(priority) << "] " << data_point.get_address() << ": ";
...
std::string SyslogSink::format_timestamp_rfc5424() const {
    ...
    std::ostringstream oss;
    oss << std::put_time(&tm_now, "%Y-%m-%dT%H:%M:%S") << "." << std::setfill('0') << std::setw(3)
        << ms.count() << "Z";
    return oss.str();
}
...
common::Result<void> SyslogSink::send_to_remote_syslog(const std::string& message) {
    ...
    if (remote_socket_ == -1) {
        return common::Result<void>(common::ErrorCode::CONNECTION_CLOSED,
                                    "Remote socket not connected");
    }

    ssize_t sent = send(remote_socket_, message.c_str(), message.length(), 0);
    if (sent < 0) {
        return common::Result<void>(common::ErrorCode::WRITE_ERROR,
                                    "Failed to send to remote syslog");
    }
...
common::Result<void> SyslogSink::send_to_fallback(const std::string& message) {
    ...
    if (config_.fallback_config.enable_file_fallback && fallback_file_stream_ &&
        fallback_file_stream_->is_open()) {
...
std::string SyslogSink::format_json(const common::DataPoint& data_point,
                                    SyslogPriority priority) const {
    Json::Value json;
...
common::Result<void> SyslogSink::send_to_local_syslog(const std::string& message,
...
common::Result<void> SyslogSink::establish_remote_connection() {
    ...
    if (remote_socket_ == -1) {
        return common::Result<void>(common::ErrorCode::CONNECTION_FAILED,
    ...
    if (!host) {
        close(remote_socket_);
        remote_socket_ = -1;
        return common::Result<void>(common::ErrorCode::DNS_RESOLUTION_FAILED,
                                    "Failed to resolve hostname: " + config_.remote_host);
    }
    ...
    if (config_.transport != SyslogTransport::UDP) {
        if (connect(remote_socket_, (struct sockaddr*)&addr, sizeof(addr)) == -1) {
            ...
            remote_socket_ = -1;
            return common::Result<void>(common::ErrorCode::CONNECTION_FAILED,
                                        "Failed to connect to remote syslog server");
        }
    }
