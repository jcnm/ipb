 ...
 */
enum class MessageSecurityMode : uint8_t { NONE = 1, SIGN = 2, SIGN_AND_ENCRYPT = 3 };
...
public:
    ...
    ipb::common::Result<> write_values(
        const std::vector<std::pair<NodeId, ipb::common::Value>>& values);

    ipb::common::Result<ipb::common::Value> read_value(const NodeId& node_id);
    ipb::common::Result<std::vector<ipb::common::Value>> read_values(
    ...
    ipb::common::Result<ipb::common::DataSet> read_historical_data(
        const NodeId& node_id, ipb::common::Timestamp start_time, ipb::common::Timestamp end_time,
    ...
    ipb::common::Result<uint32_t> add_monitored_item(uint32_t subscription_id,
                                                     const NodeId& node_id);
    ipb::common::Result<> remove_monitored_item(uint32_t subscription_id,
...
private:
    ...
    static void subscription_inactivity_callback(UA_Client* client, UA_UInt32 subId,
                                                 void* subContext);

    static void delete_subscription_callback(UA_Client* client, UA_UInt32 subId, void* subContext);

    static void data_change_notification_callback(UA_Client* client, UA_UInt32 subId,
                                                  void* subContext, UA_UInt32 monId,
                                                  void* monContext, UA_DataValue* value);

    // Batch optimization
    ipb::common::Result<ipb::common::DataSet> read_batch_optimized(
        const std::vector<NodeId>& node_ids);
};
