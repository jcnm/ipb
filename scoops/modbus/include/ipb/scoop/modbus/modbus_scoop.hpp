...
public:
    ...
    ipb::common::Result<std::vector<bool>> read_coils(uint8_t slave_id, uint16_t address,
                                                      uint16_t count);
    ipb::common::Result<std::vector<bool>> read_discrete_inputs(uint8_t slave_id, uint16_t address,
                                                                uint16_t count);
    ipb::common::Result<std::vector<uint16_t>> read_input_registers(uint8_t slave_id,
                                                                    uint16_t address,
                                                                    uint16_t count);
    ipb::common::Result<std::vector<uint16_t>> read_holding_registers(uint8_t slave_id,
                                                                      uint16_t address,
                                                                      uint16_t count);
...
private:
    ...
    ipb::common::Value convert_raw_data(const std::vector<uint16_t>& raw_data, DataType data_type,
                                        uint16_t count);
...
public:
    ...
    static std::unique_ptr<ModbusScoop> create_rtu(const std::string& device, int baud_rate = 9600,
                                                   char parity = 'N', int data_bits = 8,
                                                   int stop_bits = 1);

    // Preset factories
    static std::unique_ptr<ModbusScoop> create_high_performance_tcp(const std::string& host,
                                                                    uint16_t port);
    static std::unique_ptr<ModbusScoop> create_low_latency_tcp(const std::string& host,
                                                               uint16_t port);
