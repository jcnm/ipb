...
class DataSet {
    public:
        ...
        explicit DataSet(size_t capacity) {
        ...
        DataSet filter_by_protocol(uint16_t protocol_id) const {
            ...
            std::copy_if(this->begin(), this->end(), std::back_inserter(result.data_points_),
                        [protocol_id](const DataPoint& dp) {
                            return dp.protocol_id() == protocol_id;
                        });
        ...
        DataSet filter_by_address_prefix(std::string_view prefix) const {
            ...
            std::copy_if(begin(), end(), std::back_inserter(result.data_points_),
                        [prefix](const DataPoint& dp) {
                            return dp.address().starts_with(prefix);
                        });
        ...
        DataSet filter_by_quality(Quality min_quality) const {
            ...
            std::copy_if(begin(), end(), std::back_inserter(result.data_points_),
                        [min_quality](const DataPoint& dp) {
                            return dp.quality() >= min_quality;
                        });
        ...
        size_t valid_count() const {
            return std::count_if(begin(), end(), [](const DataPoint& dp) {
                return dp.is_valid();
            });
        }
