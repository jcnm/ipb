...
bool should_use_colors() noexcept {
    ...
    return platform::get_env("WT_SESSION").length() > 0 ||
           platform::get_env("ConEmuANSI") == "ON";
#else
...
void ConsoleSink::write(const LogRecord& record) {
    ...
    std::ostream& out = (config_.use_stderr && record.level >= LogLevel::ERROR)
                        ? std::cerr : std::cout;
    ...
    if (config_.include_timestamp) {
        ...
        if (config_.use_colors) out << DIM;
        out << std::put_time(std::localtime(&time), "%Y-%m-%d %H:%M:%S")
            << '.' << std::setfill('0') << std::setw(3) << ms.count();
        if (config_.use_colors) out << RESET;
        out << ' ';
    }
    ...
    if (!record.category.empty()) {
        if (config_.use_colors) out << "\033[34m";  // Blue
        out << '[' << record.category << ']';
        if (config_.use_colors) out << RESET;
        out << ' ';
    }
...
FileSink::FileSink(Config config)
...
void FileSink::write(const LogRecord& record) {
    ...
    if (impl_->config.max_file_size > 0 &&
        impl_->current_size >= impl_->config.max_file_size) {
        impl_->rotate();
    }
}
...
void Logger::log(LogLevel level,
                 ...
                 std::string message,
                 SourceLocation loc) {
    log(level, category, std::move(message),
        TraceScope::current_trace_id(),
        TraceScope::current_span_id(),
        loc);
}
...
TraceScope::TraceScope(TraceId trace_id)
    : TraceScope(trace_id, SpanId::generate()) {}

TraceScope::TraceScope(TraceId trace_id, SpanId span_id)
    : trace_id_(trace_id)
    , span_id_(span_id) {
...
Span::Span(std::string_view name,
    ...
    , category_(category)
    , location_(loc)
    , trace_id_(TraceScope::current_trace_id())
    , span_id_(SpanId::generate())
    , parent_span_id_(TraceScope::current_span_id())
...
Span::Span(std::string_view name,
           ...
           SourceLocation loc)
    : name_(name)
    , category_(parent.category_)
    , location_(loc)
    , trace_id_(parent.trace_id_)
    , span_id_(SpanId::generate())
...
Span::~Span() {
    ...
    if (has_error_) {
        IPB_LOG_ERROR(category_,
            "Span completed with error: " << name_ <<
            " duration=" << us << "us" <<
            " error=" << error_name(error_code_) <<
            (error_message_.empty() ? "" : " msg=") << error_message_);
    } else {
        IPB_LOG_DEBUG(category_,
            "Span completed: " << name_ << " duration=" << us << "us");
    }
}
