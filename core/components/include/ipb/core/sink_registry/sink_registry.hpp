...
struct SinkRegistryStats {
    ...
    SinkRegistryStats& operator=(const SinkRegistryStats& other) {
        if (this != &other) {
            ...
            active_sinks.store(other.active_sinks.load());
            healthy_sinks.store(other.healthy_sinks.load());
            degraded_sinks.store(other.degraded_sinks.load());
            unhealthy_sinks.store(other.unhealthy_sinks.load());
        }
        return *this;
    }
