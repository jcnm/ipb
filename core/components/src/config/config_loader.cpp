...
ConnectionConfig parse_connection_config(const YAML::Node& node) {
    ...
    config.keepalive_interval =
        yaml_get_ms(node, "keepalive_interval", std::chrono::milliseconds{60000});
    ...
    config.max_reconnect_delay =
        yaml_get_ms(node, "max_reconnect_delay", std::chrono::milliseconds{60000});
...
SchedulerConfig parse_scheduler_config(const YAML::Node& node) {
    ...
    config.default_deadline =
        std::chrono::microseconds(yaml_get<int64_t>(node, "default_deadline_us", 1000));
    ...
    config.watchdog_timeout =
        yaml_get_ms(node, "watchdog_timeout", std::chrono::milliseconds{5000});
...
MetricsConfig parse_metrics_config(const YAML::Node& node) {
    ...
    config.collection_interval =
        yaml_get_sec(node, "collection_interval", std::chrono::seconds{10});
...
ScoopConfig parse_scoop_from_yaml(const YAML::Node& node) {
    ...
    config.protocol_type =
        parse_protocol_type(yaml_get<std::string>(node, "protocol_type", "custom"));
...
RouterConfig parse_router_from_yaml(const YAML::Node& node) {
    ...
    config.routing_timeout =
        std::chrono::microseconds(yaml_get<int64_t>(node, "routing_timeout_us", 500));
...
ApplicationConfig parse_application_from_yaml(const YAML::Node& root) {
    ...
    // Platform
    config.platform = parse_platform(yaml_get<std::string>(root, "platform", "server_standard"));
...
template <>
std::string json_get<std::string>(const Json::Value& node, const std::string& key,
...
template <>
uint32_t json_get<uint32_t>(const Json::Value& node, const std::string& key,
                            uint32_t default_value) {
    return node.get(key, default_value).asUInt();
}

template <>
uint16_t json_get<uint16_t>(const Json::Value& node, const std::string& key,
...
std::chrono::milliseconds json_get_ms(const Json::Value& node, const std::string& key,
    ...
    return std::chrono::milliseconds(
        node.get(key, static_cast<int64_t>(default_value.count())).asInt64());
}
...
ConnectionConfig parse_connection_config_json(const Json::Value& node) {
    ...
    config.keepalive_interval =
        json_get_ms(node, "keepalive_interval", std::chrono::milliseconds{60000});
    ...
    config.max_reconnect_delay =
        json_get_ms(node, "max_reconnect_delay", std::chrono::milliseconds{60000});
...
MetricsConfig parse_metrics_config_json(const Json::Value& node) {
    ...
    config.enabled = json_get<bool>(node, "enabled", true);
    config.collection_interval =
        json_get_sec(node, "collection_interval", std::chrono::seconds{10});
...
ScoopConfig parse_scoop_from_json(const Json::Value& node) {
    ...
    config.protocol_type =
        parse_protocol_type(json_get<std::string>(node, "protocol_type", "custom"));
...
common::Result<std::string> ConfigLoaderImpl::read_file(const std::filesystem::path& path) {
    ...
    if (!file.is_open()) {
        return common::Result<std::string>(common::ErrorCode::OS_ERROR,
...
common::Result<void> ConfigLoaderImpl::write_file(const std::filesystem::path& path,
    ...
    if (!parent.empty() && !std::filesystem::exists(parent)) {
        ...
        if (ec) {
            return common::Result<void>(common::ErrorCode::OS_ERROR,
...
ConfigFormat ConfigLoaderImpl::resolve_format(const std::filesystem::path& path,
                                              ConfigFormat format) {
...
common::Result<ScoopConfig> ConfigLoaderImpl::load_scoop(const std::filesystem::path& path,
                                                         ConfigFormat format) {
    auto content_result = read_file(path);
    if (!content_result) {
...
common::Result<SinkConfig> ConfigLoaderImpl::load_sink(const std::filesystem::path& path,
                                                       ConfigFormat format) {
    auto content_result = read_file(path);
    if (!content_result) {
...
common::Result<RouterConfig> ConfigLoaderImpl::load_router(const std::filesystem::path& path,
                                                           ConfigFormat format) {
    auto content_result = read_file(path);
    if (!content_result) {
...
common::Result<std::vector<ScoopConfig>> ConfigLoaderImpl::load_scoops_from_directory(
    ...
    if (!std::filesystem::is_directory(dir_path)) {
        return common::Result<std::vector<ScoopConfig>>(
            common::ErrorCode::NOT_FOUND, "Directory not found: " + dir_path.string());
    }
    ...
    for (const auto& entry : std::filesystem::directory_iterator(dir_path)) {
        ...
        if (format == ConfigFormat::AUTO && !is_yaml && !is_json)
            continue;
        if (format == ConfigFormat::YAML && !is_yaml)
            continue;
        if (format == ConfigFormat::JSON && !is_json)
            continue;
...
common::Result<std::vector<SinkConfig>> ConfigLoaderImpl::load_sinks_from_directory(
    const std::filesystem::path& dir_path, ConfigFormat format) {
    if (!std::filesystem::is_directory(dir_path)) {
        return common::Result<std::vector<SinkConfig>>(common::ErrorCode::NOT_FOUND,
...
common::Result<ApplicationConfig> ConfigLoaderImpl::parse_application(std::string_view content,
                                                                      ConfigFormat format) {
    if (format == ConfigFormat::AUTO) {
...
common::Result<ScoopConfig> ConfigLoaderImpl::parse_scoop(std::string_view content,
                                                          ConfigFormat format) {
    if (format == ConfigFormat::AUTO) {
        format = detect_format_from_content(content);
    }
...
common::Result<SinkConfig> ConfigLoaderImpl::parse_sink(std::string_view content,
                                                        ConfigFormat format) {
    if (format == ConfigFormat::AUTO) {
        format = detect_format_from_content(content);
    }
...
common::Result<RouterConfig> ConfigLoaderImpl::parse_router(std::string_view content,
                                                            ConfigFormat format) {
    if (format == ConfigFormat::AUTO) {
        format = detect_format_from_content(content);
    }
...
common::Result<std::string> ConfigLoaderImpl::serialize_scoop(const ScoopConfig& /*config*/,
                                                              ConfigFormat /*format*/) {
    return common::Result<std::string>(common::ErrorCode::NOT_IMPLEMENTED,
                                       "Scoop serialization not yet implemented");
}

common::Result<std::string> ConfigLoaderImpl::serialize_sink(const SinkConfig& /*config*/,
                                                             ConfigFormat /*format*/) {
    return common::Result<std::string>(common::ErrorCode::NOT_IMPLEMENTED,
                                       "Sink serialization not yet implemented");
}

common::Result<std::string> ConfigLoaderImpl::serialize_router(const RouterConfig& /*config*/,
                                                               ConfigFormat /*format*/) {
    return common::Result<std::string>(common::ErrorCode::NOT_IMPLEMENTED,
...
common::Result<void> ConfigLoaderImpl::save_application(const ApplicationConfig& config,
...
common::Result<void> ConfigLoaderImpl::save_scoop(const ScoopConfig& config,
                                                  const std::filesystem::path& path,
                                                  ConfigFormat format) {
...
common::Result<void> ConfigLoaderImpl::save_sink(const SinkConfig& config,
                                                 const std::filesystem::path& path,
                                                 ConfigFormat format) {
