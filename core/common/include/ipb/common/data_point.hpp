...
public:
     ...
     */
    static Timestamp now() noexcept { return Timestamp(clock_type::now().time_since_epoch()); }
...
public:
    ...
    Value(const Value& other) noexcept { copy_from(other); }
    ...
    std::span<const uint8_t> as_binary() const noexcept {
        if (type_ != Type::BINARY)
            return {};
        const uint8_t* data = size_ <= INLINE_SIZE ? inline_data_ : external_data_.get();
    ...
    bool operator==(const Value& other) const noexcept {
        ...
        switch (type_) {
            ...
            case Type::INT8:
                return get<int8_t>() == other.get<int8_t>();
            case Type::INT16:
    ...
    size_t serialized_size() const noexcept { return sizeof(Type) + sizeof(size_t) + size_; }
...
private:
    ...
    static constexpr bool is_supported_type_v =
        std::is_same_v<T, bool> || std::is_same_v<T, int8_t> || std::is_same_v<T, int16_t> ||
...
public:
    ...
    DataPoint(const DataPoint& other) noexcept { copy_from(other); }
    ...
    void set_address(std::string_view address) noexcept {
        ...
        if (address_size_ <= MAX_INLINE_ADDRESS) {
            ...
            new (&external_address_)
                std::unique_ptr<char[]>(std::make_unique<char[]>(address_size_));
    ...
    bool operator==(const DataPoint& other) const noexcept {
        return address() == other.address() && protocol_id_ == other.protocol_id_;
    }
...
public:
    ...
    explicit RawMessage(std::span<const uint8_t> data) noexcept : data_(data), owns_data_(false) {}
...
struct hash<ipb::common::DataPoint> {
    size_t operator()(const ipb::common::DataPoint& dp) const noexcept { return dp.hash(); }
};
}  // namespace std
