...
struct Statistics {
    ...
    double messages_per_second() const noexcept {
        ...
        return seconds.count() > 0 ? static_cast<double>(total_messages) / seconds.count() : 0.0;
    }

    double average_processing_time_us() const noexcept {
        return total_messages > 0
                 ? static_cast<double>(total_processing_time.count()) / total_messages / 1000.0
                 : 0.0;
    }
...
public:
    ...
    std::unique_ptr<ConfigurationBase> get_configuration() const {
        return impl_->get_configuration();
    }
...
public:
    ...
    IIPBSink(std::unique_ptr<T> impl) : impl_(std::move(impl)) {
        static_assert(std::is_base_of_v<IIPBSinkBase, T>, "T must inherit from IIPBSinkBase");
    }
...
public:
    ...
    virtual Result<IProtocolSource> create_protocol_source(std::string_view protocol_name,
                                                           const ConfigurationBase& config) = 0;

    // Sink creation
    virtual Result<IIPBSink> create_sink(std::string_view sink_type,
                                         const ConfigurationBase& config) = 0;
