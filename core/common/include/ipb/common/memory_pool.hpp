...
public:
    ...
    size_t capacity() const noexcept {
        return stats_.capacity.load(std::memory_order_relaxed);
    }
    ...
    size_t in_use() const noexcept {
        return stats_.in_use.load(std::memory_order_relaxed);
    }
    ...
    void allocate_block() {
        ...
        for (size_t i = 0; i < BlockSize; ++i) {
            Node* node = reinterpret_cast<Node*>(
                static_cast<char*>(memory) + i * object_size);
...
private:
    ...
    struct SmallBlock { alignas(64) char data[SMALL_SIZE]; };
    struct MediumBlock { alignas(64) char data[MEDIUM_SIZE]; };
    struct LargeBlock { alignas(64) char data[LARGE_SIZE]; };
