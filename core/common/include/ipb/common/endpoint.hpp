...
public:
    ...
    EndPoint(Protocol protocol, std::string_view path) : protocol_(protocol), path_(path) {}

    // Constructor with full parameters
    EndPoint(Protocol protocol, std::string_view host, uint16_t port, std::string_view path,
             SecurityLevel security = SecurityLevel::NONE)
    ...
    std::chrono::milliseconds connection_timeout() const noexcept { return connection_timeout_; }

    void set_read_timeout(std::chrono::milliseconds timeout) noexcept { read_timeout_ = timeout; }

    std::chrono::milliseconds read_timeout() const noexcept { return read_timeout_; }

    void set_write_timeout(std::chrono::milliseconds timeout) noexcept { write_timeout_ = timeout; }

    std::chrono::milliseconds write_timeout() const noexcept { return write_timeout_; }
    ...
    bool operator==(const EndPoint& other) const noexcept {
        return protocol_ == other.protocol_ && host_ == other.host_ && port_ == other.port_ &&
               path_ == other.path_;
    }
...
struct ConnectionStats {
    ...
    void reset() noexcept { *this = ConnectionStats{}; }

    double connection_success_rate() const noexcept {
        return connection_attempts > 0
                 ? static_cast<double>(successful_connections) / connection_attempts * 100.0
                 : 0.0;
    }
    ...
    double uptime_percentage(std::chrono::steady_clock::time_point start_time) const noexcept {
        ...
        return total_time.count() > 0
                 ? static_cast<double>(total_connected_time.count()) / total_time.count() * 100.0
                 : 0.0;
    }
};
...
public:
    ...
    void release(T* ptr) noexcept {
        ...
        } while (!free_list_.compare_exchange_weak(current_head, node, std::memory_order_release,
...
public:
    ...
    duration elapsed() const noexcept { return clock_type::now() - start_time_; }
