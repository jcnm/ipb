...
void Value::serialize(std::span<uint8_t> buffer) const noexcept {
    if (buffer.size() < serialized_size()) return;
    ...
    // Write data
    if (size_ > 0) {
        const uint8_t* data_ptr = size_ <= INLINE_SIZE ? 
...
uint32_t Value::get_impl<uint32_t>() const noexcept {
    if (type_ != Type::UINT32 || size_ != sizeof(uint32_t)) return 0;
    uint32_t result;
...
int64_t Value::get_impl<int64_t>() const noexcept {
    if (type_ != Type::INT64 || size_ != sizeof(int64_t)) return 0;
    int64_t result;
...
uint64_t Value::get_impl<uint64_t>() const noexcept {
    if (type_ != Type::UINT64 || size_ != sizeof(uint64_t)) return 0;
    uint64_t result;
...
float Value::get_impl<float>() const noexcept {
    if (type_ != Type::FLOAT32 || size_ != sizeof(float)) return 0.0f;
    float result;
...
double Value::get_impl<double>() const noexcept {
    if (type_ != Type::FLOAT64 || size_ != sizeof(double)) return 0.0;
    double result;
...
void DataPoint::serialize(std::span<uint8_t> buffer) const noexcept {
    ...
    const char* addr_data = address_size_ <= MAX_INLINE_ADDRESS ?
...
bool DataPoint::deserialize(std::span<const uint8_t> buffer) noexcept {
    ...
    // Read value
    if (!value_.deserialize(std::span<const uint8_t>(buffer.data() + offset, buffer.size() - offset))) {
        return false;
    }
    offset += value_.serialized_size();
    
    if (buffer.size() < offset + sizeof(int64_t) + sizeof(uint16_t) + sizeof(Quality) + sizeof(uint32_t)) {
        return false;
    }
