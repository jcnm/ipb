...
create_modbus_adapters() {
    ...
    // Production Line 1 - Temperature and Pressure Monitoring
    {
        ...
        config.registers.push_back({.name      = "line1_flow_rate",
                                    .address   = 40005,
                                    .type      = adapter::modbus::RegisterType::HOLDING_REGISTER,
                                    .data_type = adapter::modbus::DataType::FLOAT32,
        ...
        config.coils.push_back(
            {.name = "line1_pump_status", .address = 10001, .polling_interval = 50ms});

        config.coils.push_back(
            {.name = "line1_maintenance_mode", .address = 10002, .polling_interval = 1s});
    ...
    // Production Line 2 - Vibration and Speed Monitoring
    {
        ...
        config.registers.push_back({.name      = "line2_vibration_y",
                                    .address   = 40003,
                                    .type      = adapter::modbus::RegisterType::HOLDING_REGISTER,
                                    .data_type = adapter::modbus::DataType::FLOAT32,
                                    .polling_interval = 10ms});

        config.registers.push_back({.name      = "line2_motor_speed",
                                    .address   = 40005,
                                    .type      = adapter::modbus::RegisterType::HOLDING_REGISTER,
                                    .data_type = adapter::modbus::DataType::INT16,
...
std::shared_ptr<common::IProtocolSource> create_opcua_adapter() {
    ...
    config.node_ids = {adapter::opcua::NodeId::parse("ns=2;s=Process.Temperature.Reactor1"),
 ...
 */
std::vector<std::pair<std::string, std::shared_ptr<common::IIPBSink>>> create_data_sinks() {
...
void print_system_status(const gate::Orchestrator& orchestrator) {
    while (orchestrator.is_running()) {
        ...
        std::cout << "Memory Usage: " << (metrics.system_memory_usage / 1024 / 1024) << " MB"
                  << std::endl;
        ...
        for (const auto& [component_id, component_info] : metrics.component_metrics) {
            std::cout << "  " << component_id << ": " << component_info.successful_operations
                      << " ops, " << "avg " << component_info.avg_processing_time.count() << "ns"
                      << std::endl;
        }
...
int main(int argc, char* argv[]) {
    try {
        ...
        if (!init_result.is_success()) {
            std::cerr << "Failed to initialize orchestrator: " << init_result.error_message()
                      << std::endl;
            return 1;
        }
        ...
        if (!router_register_result.is_success()) {
            std::cerr << "Failed to register router: " << router_register_result.error_message()
                      << std::endl;
            return 1;
        }
        ...
        if (!start_result.is_success()) {
            std::cerr << "Failed to start orchestrator: " << start_result.error_message()
                      << std::endl;
            return 1;
        }
        ...
        if (!stop_result.is_success()) {
            std::cerr << "Warning: Error during shutdown: " << stop_result.error_message()
                      << std::endl;
        }
        ...
        if (!shutdown_result.is_success()) {
            std::cerr << "Warning: Error during final shutdown: " << shutdown_result.error_message()
                      << std::endl;
        }
