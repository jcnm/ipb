...
public:
    MockIndustrialSource(const std::string& protocol_id, const std::string& base_address,
                         std::chrono::milliseconds update_interval)
        : protocol_id_(protocol_id), base_address_(base_address), update_interval_(update_interval),
          running_(false), generator_(std::random_device{}()), temperature_dist_(15.0, 35.0),
          pressure_dist_(1.0, 5.0), flow_dist_(10.0, 100.0), bool_dist_(0.0, 1.0) {}

    ~MockIndustrialSource() { stop(); }
...
public:
    ...
    void start() {
        ...
        std::cout << "Started statistics monitor (interval: " << interval_.count() << "s)"
                  << std::endl;
    }
    ...
    void add_sink(const std::string& name, common::IIPBSink* sink) { sinks_[name] = sink; }

    void add_router(router::IPBRouter* router) { router_ = router; }
...
private:
    ...
    void print_statistics(std::chrono::seconds elapsed) {
        std::cout << "\n=== IPB System Statistics (last " << elapsed.count()
                  << "s) ===" << std::endl;

        // Source statistics
        std::cout << "\nData Sources:" << std::endl;
        for (const auto& [name, source] : sources_) {
            std::cout << "  " << name << ": " << source->get_messages_generated()
                      << " messages generated" << std::endl;
        }
        ...
        if (router_) {
            ...
            std::cout << "  Avg processing time: " << router_metrics.avg_processing_time.count()
                      << "ns" << std::endl;
        }
        ...
        for (const auto& [name, sink] : sinks_) {
            ...
            std::cout << "    Avg processing time: " << metrics.avg_processing_time.count() << "ns"
                      << std::endl;
        }
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "This example demonstrates data flow from mock sources through the IPB router to "
                 "MQTT and Console sinks."
              << std::endl;
    ...
    try {
        ...
        if (!mqtt_init_result.is_success()) {
            std::cout << "Warning: Failed to initialize MQTT sink: " << mqtt_init_result.get_error()
                      << std::endl;
            ...
            if (!mqtt_start_result.is_success()) {
                std::cout << "Warning: Failed to start MQTT sink: " << mqtt_start_result.get_error()
                          << std::endl;
                mqtt_sink.reset();
            } else {
        ...
        if (!console_init_result.is_success()) {
            std::cerr << "Failed to initialize Console sink: " << console_init_result.get_error()
                      << std::endl;
            return 1;
        }
        ...
        if (!console_start_result.is_success()) {
            std::cerr << "Failed to start Console sink: " << console_start_result.get_error()
                      << std::endl;
            return 1;
        }
        ...
        if (!router_init_result.is_success()) {
            std::cerr << "Failed to initialize router: " << router_init_result.get_error()
                      << std::endl;
            return 1;
        }
        ...
        if (mqtt_sink) {
            auto mqtt_register_result =
                ipb_router->register_sink("mqtt_industrial", mqtt_sink.get());
        ...
        auto console_register_result =
            ipb_router->register_sink("console_debug", console_sink.get());
        if (!console_register_result.is_success()) {
            std::cerr << "Failed to register Console sink: " << console_register_result.get_error()
                      << std::endl;
            return 1;
        }
        ...
        if (!temp_rule_result.is_success()) {
            std::cerr << "Failed to add temperature routing rule: " << temp_rule_result.get_error()
                      << std::endl;
        }
        ...
        if (mqtt_sink) {
            alarm_rule.destinations.push_back(
                {"mqtt_industrial", router::RoutingPriority::CRITICAL});
        }
        ...
        if (!alarm_rule_result.is_success()) {
            std::cerr << "Failed to add alarm routing rule: " << alarm_rule_result.get_error()
                      << std::endl;
        }
        ...
        if (!default_rule_result.is_success()) {
            std::cerr << "Failed to add default routing rule: " << default_rule_result.get_error()
                      << std::endl;
        }
        ...
        if (!router_start_result.is_success()) {
            std::cerr << "Failed to start router: " << router_start_result.get_error() << std::endl;
            return 1;
        }
        ...
        auto modbus_source = std::make_unique<MockIndustrialSource>("modbus", "plant_a/line_1",
                                                                    std::chrono::milliseconds{500});

        // OPC UA source (medium updates)
        auto opcua_source = std::make_unique<MockIndustrialSource>("opcua", "plant_b/reactor_1",
                                                                   std::chrono::milliseconds{1000});

        // MQTT source (slow updates)
        auto mqtt_source = std::make_unique<MockIndustrialSource>("mqtt", "plant_c/warehouse",
        ...
        std::cout << "Check the console output and /tmp/ipb_test_output.log for results."
                  << std::endl;
        if (mqtt_sink) {
